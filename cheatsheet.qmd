...
## Entry points by level (@sec-intro)

| Level | Call | Chapter |
|-------|------|---------|
| L1 | `ptr_app(formula, envir, ui_text, expr_check, safe_to_remove, css, spec)` | @sec-intro |
...
`ptr_normalize_column_names(data)` — make local data `ppVar`-safe (@sec-getting-started). `ptr_extract_code(state)` — the script behind the code window (@sec-generated-code).
