...
Now replace each tuning knob with a placeholder. The columns become pickers, the labels become text inputs, the y-range becomes a slider backed by a small custom value placeholder, `ppRange`, defined right in the chunk:
...
ggpaintr parses any expression that evaluates to a ggplot object, including the common "clean, then plot" pipeline. Pipeline stages with placeholders get the per-layer **Data** subtab described in @sec-formula-language.
...
A classic exploratory move — project numeric columns onto their first two principal components, colour by a grouping factor, overlay confidence ellipses. The `do_pca()` helper keeps the lambda out of the pipe (anonymous-function pipe stages are unsupported — @sec-formula-language):
...
`lincoln_weather` ships with non-syntactic column names (`Mean Temperature [F]`, …), so a parameterized version needs `ptr_normalize_column_names()` first — the recipe is in @sec-use-cases.
...
- The three-level ladder (all-in-one app → embed → own the layout): @sec-use-cases, @sec-embed-default, @sec-embed-bare.
