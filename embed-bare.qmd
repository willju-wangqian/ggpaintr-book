...
The pieces are deliberately orthogonal — an `id` (and, where relevant, a `formula`) and nothing else. There are no `error=` / `code_toggle=` flags on `ptr_ui_plot()`; layered behaviour is added by **combinators** that wrap already-built pieces:
...
Want the `</>` slide-out and inline error while owning the surrounding layout? Compose with the two combinators — this nested call is byte-for-byte the output block `ptr_app()` and `ptr_ui()` render internally, so behaviour matches the all-in-one path:
