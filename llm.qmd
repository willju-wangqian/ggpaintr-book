...
Topics whose example is a standalone runnable app open with a `library()` preamble; signature- or fragment-only topics (`level1_ptr_app`, `level1_ptr_options`, `overview`) do not, by design.
...
- **Changing the tool name** — rebuild the chat from scratch; `Chat` objects do not expose a public unregister API in current ellmer.

## Current behavior boundary

- The primer and topic files are bundled with the package and hand-curated; regenerating them from the source documentation is not yet automated.
