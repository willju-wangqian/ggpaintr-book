...
| `ppUpload`       | The `ppUpload` widget — file-picker label, optional dataset-name label / placeholder / help text. |
...
A custom placeholder's registration-time `ui_text_defaults` (@sec-custom-placeholders) populates its entry in the `defaults` section, so app authors can re-copy *your* widget through the same `ui_text =` machinery without touching your code — the `{param}` interpolation included.
